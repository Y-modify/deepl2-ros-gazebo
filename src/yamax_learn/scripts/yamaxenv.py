...
class YamaXSimEnv(gazebo_env.GazeboEnv):
    def __init__(self):
        gazebo_env.GazeboEnv.__init__(
            self, "/yamax/src/yamax_gazebo/launch/world.launch")
        self.reset_proxy = rospy.ServiceProxy(
            '/gazebo/reset_simulation', Empty)
        self.unpause = rospy.ServiceProxy('/gazebo/unpause_physics', Empty)
        self.pause = rospy.ServiceProxy('/gazebo/pause_physics', Empty)
        self.joints = ["neck", "shoulder_right_x", "shoulder_right_z", "shoulder_left_x", "shoulder_left_z", "elbow_right", "elbow_left", "backbone_1", "backbone_2",
        ...
        # 観測空間(state)の次元 (関節数+加速度+位置) とそれらの最大値(1)
        high = np.array([1.57] * len(self.joints) + [1] + [10])
        self.observation_space = gym.spaces.Box(
            low=-high, high=high)  # 最小値は、最大値のマイナスがけ
    ...
    def read_sensors(self):
        ...
        while acc is None:
            try:
                acc = rospy.wait_for_message(
                    '/yamax/imu', Imu, timeout=5).linear_acceleration.x
            except:
                pass
        ...
        while pos is None:
            try:
                pos = rospy.wait_for_message(
                    '/yamax/odom', Odometry, timeout=5).pose.pose.position.x
            except:
                pass
